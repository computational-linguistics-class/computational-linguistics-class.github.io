...
$$\frac{\sum_C \log p(c_{POS}) - \log p(c_{NEG})}{|C|}$$

where $$C$$ is the list of characters in a tweet, and $$\log p(c_{X})$$ gives the log probability of the tweet under language model $$X$$.
