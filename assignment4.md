...
##  Part 3: SimLex-999 Dataset (40 points)
[SimLex-999](https://fh295.github.io/simlex.html)


1) Download the SimLex-999 data from https://fh295.github.io/simlex.html
2) Try to read it in to your Python program using pandas or a CSV reader
3) Get the word1, word2, and SimLex columns
4) Use magnitude to create vectors for word1 and word2 and compare their cosine similarity
5) Implement or find a Kendall’s Tau correlation function (see https://en.wikipedia.org/wiki/Kendall_rank_correlation_coefficient)
6) Compute the correlation for human judgments of similarity (the SimLex scores) to the cosine similarities.
7) Does that number change for word2vec versus Glove?  For 50-dimensions v 300?
...
In Part 2 we made the clustering problem deliberately easier by providing you with `k`, the number of clusters, as an input. But in most clustering situations the best `k` is not given.
To take this assignment one step further, see if you can come up with a way to automatically choose `k`. We have provided an additional test set, `test_nok_input.txt`, where the `k` field has been zeroed out. See if you can come up with a method that clusters words by sense, and chooses the best `k` on its own. 
(Don't look at the number of WordNet synsets for this, as that would ruin all the fun.) The baseline system for this portion always chooses `k=5`.
...
* `part1.txt` file with answers to questions from Exploration
</div>
